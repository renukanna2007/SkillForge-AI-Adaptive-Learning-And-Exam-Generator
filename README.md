...
- **Student portal**
  ...
  - View scores, attempt history, and detailed results.
...
- **Admin / faculty portal**
  ...
  - Manage question banks and exam configurations.[file:2707]

- **AI exam generation**
  - Uses LM Studio running a local model (e.g., `tinyllama-1.1b-chat-v1.0`).[file:2707]
...
- **AI Tutor**
  - `/api/ai-tutor/chat` endpoint connects to the local LLM via the OpenAI-compatible `/v1/chat/completions` API.[file:2707]
  - Chat-style interface where students can ask course-related questions (“define Java”, “explain DBMS”, etc.).
  - Short, focused explanations targeted at B.Tech IT level.
...
- [LM Studio](https://lmstudio.ai/) installed and a compatible chat model downloaded (e.g., `tinyllama-1.1b-chat-v1.0`).[file:2707]
